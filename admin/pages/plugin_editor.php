...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['file_path']) && isset($_POST['file_content'])) {
    ...
    // Security check: Ensure the file being edited is inside the plugins directory
    if ($filePath && strpos($filePath, $pluginsDir) === 0) {
        ...
        $msg = "Đường dẫn tệp không hợp lệ hoặc nằm ngoài thư mục plugins!";
        $msgType = 'error';
    }
}
    ...
    foreach ($cdir as $key => $value) {
        if (!in_array($value, array(".", ".."))) {
            if (is_dir($dir . DIRECTORY_SEPARATOR . $value)) {
                $result[$value] = scanPluginsDir($dir . DIRECTORY_SEPARATOR . $value);
            } else {
...
$pluginTree = scanPluginsDir($pluginsDir);

// Render Tree HTML
function renderTree($tree, $basePath = 'plugins') {
    ...
    foreach ($tree as $key => $value) {
        if (is_array($value)) {
            ...
            $activeClass = (isset($_GET['file']) && $_GET['file'] === $currentPath) ? 'text-blue-400 font-bold' : 'text-gray-400 hover:text-white';
            $html .= '<li class="my-1">';
            $html .= '<a href="?page=plugin_editor&file=' . urlencode($currentPath) . '" class="flex items-center text-sm transition-colors ' . $activeClass . '">';
...
if ($selectedFile) {
    ...
    if ($fullPath && strpos($fullPath, $pluginsDir) === 0 && file_exists($fullPath)) {
...
<div class="mb-6 flex justify-between items-center">
    <div>
        <h2 class="text-2xl font-bold text-white mb-2 flex items-center gap-2">
            <a href="?page=plugins" class="text-gray-400 hover:text-white transition-colors" title="Trở lại Quản Lý Plugin">
                <i data-lucide="arrow-left" class="w-6 h-6"></i>
            </a>
            Sửa Code Plugin (Plugin Editor)
        </h2>
        <p class="text-gray-400 text-sm">Chỉnh sửa trực tiếp mã nguồn của các plugin đang cài đặt.</p>
    </div>
</div>
...
<div class="flex flex-col md:flex-row gap-6 h-[calc(100vh-200px)] min-h-[600px]">
    ...
    <div class="w-full md:w-1/4 h-full flex flex-col bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
        <div class="p-4 border-b border-gray-800 bg-gray-800/50">
            <h3 class="font-bold text-white flex items-center"><i data-lucide="folder-tree" class="w-4 h-4 mr-2 text-blue-400"></i> File Plugins</h3>
        </div>
        <div class="flex-1 overflow-y-auto custom-scrollbar p-4">
            <div class="text-gray-200 font-bold mb-2 flex items-center">
                <i data-lucide="folder" class="w-4 h-4 mr-2 text-yellow-500"></i> plugins
            </div>
    ...
    <div class="w-full md:w-3/4 h-full flex flex-col bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
        <?php if ($selectedFile && $isReadable): ?>
            ...
            <form method="POST" class="flex-1 flex flex-col h-full">
                ...
                <div class="p-4 border-t border-gray-800 bg-gray-900 flex justify-end">
                    <?php if ($isWritable): ?>
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-8 rounded-lg transition-colors flex items-center">
        ...
        <?php else: ?>
            <div class="flex flex-col items-center justify-center h-full text-center p-12">
                ...
                <h3 class="text-xl font-bold text-gray-400 mb-2">Trình Sửa Plugin</h3>
