...
?>
    <div class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 mt-4 mb-8">
        ...
        ?>
            ...
            <?php if ($is_watching): ?>
                ...
                <div class="bg-black border border-gray-800 rounded-xl overflow-hidden shadow-2xl mb-6">
                    <div id="player-container" class="aspect-video w-full bg-[#0a0a0a] flex items-center justify-center relative">
                        <div class="text-center p-8">
                            <i class="fas fa-spinner fa-spin text-4xl text-kk-red mb-4"></i>
                            <h3 class="text-white font-medium">Đang tải video...</h3>
                        </div>
                    </div>
                    <div class="p-4 bg-[#181a20] flex items-center justify-between border-t border-gray-800">
                        <h1 class="text-xl font-bold text-white"><?php the_title(); ?> <span id="current-ep-title" class="text-gray-400 text-base font-normal"></span></h1>
                ...
                <!-- DANH SÁCH TẬP -->
                <div id="episode-list-container" class="bg-[#181a20] rounded-xl border border-gray-800 p-6 shadow-xl mb-6 hidden">
                    <h3 class="text-white font-bold text-lg mb-4 border-l-4 border-kk-red pl-3 uppercase">Danh Sách Tập</h3>
                    <div id="servers-container" class="space-y-6"></div>
                </div>

                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        const slug = '<?php echo esc_js($slug); ?>';
                        const requestedEp = '<?php echo esc_js($ep); ?>';
                        const apiUrl = 'http://103.72.97.151:3000/api/phim/' + slug;
                        
                        fetch(apiUrl)
                            .then(res => res.json())
                            .then(data => {
                                if (data.status === 'success' && data.episodes && data.episodes.length > 0) {
                                    renderPlayer(data.episodes, requestedEp);
                                } else {
                                    showError('Không tìm thấy dữ liệu tập phim.');
                                }
                            })
                            .catch(err => {
                                showError('Lỗi kết nối đến máy chủ API: ' + err.message);
                            });
                        ...
                        function showError(msg) {
                            document.getElementById('player-container').innerHTML = `
                                <div class="text-center p-8">
                                    ...
                                    <h3 class="text-white font-medium mb-2">Lỗi tải phim</h3>
                                    <p class="text-gray-400 text-sm">${msg}</p>
                                </div>
                            `;
                        }
                        
                        function renderPlayer(episodes, requestedEp) {
                            // Find the episode to play
                            let epToPlay = null;
                            let firstEp = episodes[0].server_data[0];
                            
                            if (requestedEp) {
                                for (let srv of episodes) {
                                    let found = srv.server_data.find(e => e.slug === requestedEp);
                                    if (found) { epToPlay = found; break; }
                                }
                            }
                            if (!epToPlay) epToPlay = firstEp;
                            
                            // Render Iframe
                            document.getElementById('player-container').innerHTML = `
                                <iframe src="${epToPlay.link_embed}" class="w-full h-full border-0" allowfullscreen></iframe>
                            `;
                            document.getElementById('current-ep-title').textContent = ' - ' + epToPlay.name;
                            
                            // Render Episode List
                            const srvContainer = document.getElementById('servers-container');
                            let html = '';
                            episodes.forEach(srv => {
                                html += `<div>
                                    <h4 class="text-gray-400 text-sm mb-3"><i class="fas fa-server mr-2"></i>${srv.server_name}</h4>
                                    <div class="flex flex-wrap gap-2">
                                `;
                                srv.server_data.forEach(ep => {
                                    const isCurrent = (ep.slug === epToPlay.slug);
                                    const btnClass = isCurrent 
                                        ? 'bg-kk-red text-white border-kk-red' 
                                        : 'bg-gray-800 text-gray-300 border-gray-700 hover:bg-gray-700 hover:text-white hover:border-gray-500';
                                        
                                    html += `<a href="?watch=true&ep=${ep.slug}" class="px-4 py-2 rounded border text-sm font-medium transition ${btnClass}">${ep.name}</a>`;
                                });
                                html += `</div></div>`;
                            });
                            
                            srvContainer.innerHTML = html;
                            document.getElementById('episode-list-container').classList.remove('hidden');
                        }
                    });
                </script>
            <?php endif; ?>
