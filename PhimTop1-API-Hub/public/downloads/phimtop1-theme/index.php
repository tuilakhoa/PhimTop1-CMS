<?php get_header(); ?>
    <div class="max-w-[1440px] mx-auto px-4 py-8 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between mb-8 border-b border-gray-800 pb-4">
            <h2 class="text-2xl font-bold text-white border-l-4 border-kk-red pl-3 uppercase">Phim Mới Cập Nhật</h2>
        </div>
        ...
        <?php endwhile; wp_reset_postdata(); else: ?>
            <div class="col-span-full py-20 text-center">
                ...
                <p class="text-gray-500 text-lg">Chưa có dữ liệu phim nào. Hãy dùng Plugin PhimTop1 Crawler để cào phim.</p>
            </div>
        <?php endif; ?>
        </div>
            ...
            <div class="inline-flex items-center gap-1 bg-[#181a20] p-1 rounded-lg border border-gray-800">
                <?php 
                    echo paginate_links(array(
                        ...
                        'type' => 'plain',
                        'before_page_number' => '<span class="px-3 py-1.5 rounded-md hover:bg-gray-800 text-gray-400 font-medium transition">',
                        'after_page_number' => '</span>'
                    ));
                ?>
            </div>
            <style>
                .page-numbers.current { background-color: #f43f5e; color: white; padding: 6px 12px; border-radius: 6px; font-weight: bold; }
                .page-numbers:not(.current) { color: #9ca3af; padding: 6px 12px; border-radius: 6px; transition: background-color 0.2s; }
                .page-numbers:not(.current):hover { background-color: #1f2937; color: white; }
            </style>
        </div>
    </div>
<?php get_footer(); ?>
