...
<script>
    var topAdsConf = <?= get_option('movie_adstop_list') ?>;
    var footerFixAds = <?= get_option('movie_footer_list') ?>;
    var overlayAds = <?= get_option('movie_overlay_list') ?>;
    document.addEventListener("DOMContentLoaded", (event) => {
        <?php
        if(get_option('movie_adstop') == 'on') { echo 'showTopads(); '; }
        if(get_option('movie_ads_footer') == 'on') { echo 'showFooterFixAds(); '; }
        if(get_option('movie_ads_overlay') == 'on') { echo 'showOverlayAds(); '; }
        ?>
    });
    <?php if(get_option('movie_ads_link') == 'on') { ?>
    document.body.addEventListener("click", function (event) {
        if (!event.target.closest('#overlay a, #overlay, #top_addd a, #top_addd, #footer_fixed_ads a, #footer_fixed_ads, center a')) {
            ...
            if(!clickads){
                window.open('<?= get_option('movie_ads_link_value') ?>');
                setCookie('clickads','clickads',<?= get_option('movie_ads_cache_time','60') ?>)
            }
...
<script>
    ...
    function showOverlayAds() {
        ...
        if(!Overlay){
            ...
            setCookie('Overlay','Overlay',<?= get_option('movie_ads_cache_time','60') ?>)
        }
