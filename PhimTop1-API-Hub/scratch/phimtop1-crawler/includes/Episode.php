...
function get_first_episode_info() {
    $episode_list = get_post_meta(get_the_id(), 'movie_episode_list', true);
...
function get_first_episode_url() {
    ...
    $getslug = get_option('movie_watch_urls') ?: 'xem-phim';
...
function watchUrl(){
    ...
    $slugView = '/'.$slug.'/';
    $post_slug = basename(get_permalink(get_the_id()));;
    $listphim = get_post_meta(get_the_id(), 'movie_episode_list', true);
...
function episodeList(){
    $episode_arr = [];
    $episode= get_post_meta(get_the_id(), 'movie_episode_list', true);
...
{
    $getslug = get_option('movie_slug_regions');
    if($getslug){
        $slug = $getslug;
    }else{
        $slug = 'regions';
    ...
    $html = "";
    $country = get_the_terms(get_the_ID(), "movie_regions");
    if (is_array($country)) {
...
{
    return get_the_terms(get_the_ID(), "movie_regions");
}

function op_get_years($end ='')
{
    $getslug = get_option('movie_slug_years');
    if($getslug){
        $slug = $getslug;
    }else{
        $slug = 'years';
    ...
    $html = "";
    $years = get_the_terms(get_the_ID(), "movie_years");
    if (is_array($years)) {
        foreach ($years as $y) {
...
{
    return get_the_terms(get_the_ID(), "movie_years");
}

function op_get_genres($end ='')
{
    $getslug = get_option('movie_slug_genres');
    if($getslug){
        $slug = $getslug;
    }else{
        $slug = 'genres';
...
{
    return get_the_terms(get_the_ID(), "movie_genres");
}

function op_get_tags($end ='')
{
    $getslug = get_option('movie_slug_tags');
    if($getslug){
        $slug = $getslug;
    }else{
        $slug = 'tags';
    ...
    $html = "";
    $tags = get_the_terms(get_the_ID(), "movie_tags");
    if (is_array($tags)) {
        foreach ($tags as $tag) {
...
function op_get_tag()
{
    return get_the_terms(get_the_ID(), "movie_tags");
}
function op_get_actors($limit = 10,$end ='')
{
    $getslug = get_option('movie_slug_actors');
    if($getslug){
        $slug = $getslug;
    }else{
        $slug = 'actors';
    ...
    $html = "";
    $actors = get_the_terms(get_the_ID(), "movie_actors");
    if (is_array($actors)) {
...
{
    $getslug = get_option('movie_slug_directors');
    if($getslug){
        $slug = $getslug;
    }else{
        $slug = 'directors';
    ...
    $html = "";
    $directors = get_the_terms(get_the_ID(), "movie_directors");
...
{
    return get_the_terms(get_the_ID(), "movie_directors");
}
...
{
    $v = get_post_meta(get_the_ID(), 'movie_preview_images', true);
    if (!is_array($v)) {
        return array();
    }
