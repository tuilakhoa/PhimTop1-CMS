...
<body class="<?= $bodyClass ?> min-h-screen">
    <nav class="glass-nav fixed w-full top-0 z-50">
                    ...
                    <a href="/" class="flex items-center space-x-2">
                        ...
                        <?php else: ?>
                            <div class="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center backdrop-blur-md">
                                <i data-lucide="monitor-play" class="w-6 h-6 text-gray-900"></i>
                            </div>
                        <?php endif; ?>
                        <span class="text-2xl font-bold tracking-tight text-gray-900"><?= htmlspecialchars($siteName) ?></span>
                    </a>
                    
                    <!-- Desktop Nav Links -->
                    <div class="hidden md:flex items-center space-x-6">
                        <a href="/<?= $settings["slugList"] ?? "danh-sach" ?>/phim-le" class="text-gray-700 hover:text-gray-900 transition-colors">Phim Lẻ</a>
                        <a href="/<?= $settings["slugList"] ?? "danh-sach" ?>/phim-bo" class="text-gray-700 hover:text-gray-900 transition-colors">Phim Bộ</a>
                        
                        <!-- Dropdown Thể Loại -->
                        <div class="relative group">
                            <button class="text-gray-700 hover:text-gray-900 transition-colors flex items-center">
                                Thể Loại <i data-lucide="chevron-down" class="w-4 h-4 ml-1"></i>
                            </button>
                            <div class="absolute left-0 mt-2 w-96 bg-gray-50 border border-gray-200 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                                <div class="p-4 grid grid-cols-3 gap-2">
                                    <?php foreach ($genres as $g): ?>
                                        <a href="/<?= $settings["slugGenre"] ?? "the-loai" ?>/<?= htmlspecialchars($g['slug']) ?>" class="text-sm text-gray-600 hover:text-gray-900 hover:bg-white px-2 py-1 rounded transition-colors truncate"><?= htmlspecialchars($g['name']) ?></a>
                                    <?php endforeach; ?>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="absolute left-0 mt-2 w-96 bg-gray-50 border border-gray-200 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                                <div class="p-4 grid grid-cols-3 gap-2">
                                    <?php foreach ($countries as $c): ?>
                                        <a href="/<?= $settings["slugCountry"] ?? "quoc-gia" ?>/<?= htmlspecialchars($c['slug']) ?>" class="text-sm text-gray-600 hover:text-gray-900 hover:bg-white px-2 py-1 rounded transition-colors truncate"><?= htmlspecialchars($c['name']) ?></a>
                                    <?php endforeach; ?>
                                </div>
                            </div>
                        </div>
                ...
                <div class="flex items-center space-x-6">
                    <form action="/search" method="GET" class="relative hidden md:block">
                        <input type="text" name="keyword" placeholder="Tìm kiếm phim..." 
                            class="bg-white/50 text-gray-200 text-sm rounded-full pl-10 pr-4 py-2 focus:outline-none focus:ring-2 focus:ring-red-500 border border-gray-300 w-64 transition-all focus:w-80">
                    ...
                    <div class="md:hidden flex items-center">
                        <button id="mobileMenuBtn" class="text-gray-700 hover:text-gray-900 focus:outline-none">
        ...
        <div id="mobileMenu" class="md:hidden hidden bg-gray-50 border-t border-gray-200 absolute w-full left-0 top-16 shadow-2xl">
            <div class="px-4 py-4 space-y-4">
                <form action="/search" method="GET" class="relative">
                    <input type="text" name="keyword" placeholder="Tìm kiếm phim..." 
                        class="w-full bg-white/50 text-gray-200 text-sm rounded-lg pl-10 pr-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-red-500 border border-gray-300">
                    <i data-lucide="search" class="absolute left-3 top-3 w-4 h-4 text-gray-600"></i>
                </form>
                <div class="flex flex-col space-y-3 font-medium text-gray-700">
                    <a href="/<?= $settings["slugList"] ?? "danh-sach" ?>/phim-le" class="hover:text-gray-900">Phim Lẻ</a>
                    <a href="/<?= $settings["slugList"] ?? "danh-sach" ?>/phim-bo" class="hover:text-gray-900">Phim Bộ</a>
                    ...
                    <div class="border-t border-gray-200 pt-2 pb-1">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <?php $limitG = array_slice($genres, 0, 8); foreach ($limitG as $g): ?>
                                <a href="/<?= $settings["slugGenre"] ?? "the-loai" ?>/<?= htmlspecialchars($g['slug']) ?>" class="text-sm text-gray-600 hover:text-gray-900 truncate"><?= htmlspecialchars($g['name']) ?></a>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    <div class="border-t border-gray-200 pt-2 pb-1">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <?php $limitC = array_slice($countries, 0, 8); foreach ($limitC as $c): ?>
                                <a href="/<?= $settings["slugCountry"] ?? "quoc-gia" ?>/<?= htmlspecialchars($c['slug']) ?>" class="text-sm text-gray-600 hover:text-gray-900 truncate"><?= htmlspecialchars($c['name']) ?></a>
                            <?php endforeach; ?>
                        </div>
                    </div>
