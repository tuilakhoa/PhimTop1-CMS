...
<div class="container mx-auto px-4 py-8">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold text-gray-900 border-l-4 border-red-500 pl-3"><?= htmlspecialchars($title) ?></h2>
    </div>
    ...
    <div class="bg-white rounded-xl p-4 mb-8">
        <form action="" method="GET" class="flex flex-wrap gap-4 items-end">
            ...
            <div class="flex-1 min-w-[150px]">
                <label class="block text-gray-600 text-xs font-medium mb-2 uppercase">Sắp xếp</label>
                <select name="sort" class="w-full bg-gray-50 border border-gray-300 text-gray-900 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-red-500 transition-colors">
            ...
            <div class="flex-1 min-w-[150px]">
                <label class="block text-gray-600 text-xs font-medium mb-2 uppercase">Thể loại</label>
                <select name="category" class="w-full bg-gray-50 border border-gray-300 text-gray-900 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-red-500 transition-colors">
            ...
            <div class="flex-1 min-w-[150px]">
                <label class="block text-gray-600 text-xs font-medium mb-2 uppercase">Quốc gia</label>
                <select name="country" class="w-full bg-gray-50 border border-gray-300 text-gray-900 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-red-500 transition-colors">
            ...
            <div class="flex-1 min-w-[120px]">
                <label class="block text-gray-600 text-xs font-medium mb-2 uppercase">Năm</label>
                <select name="year" class="w-full bg-gray-50 border border-gray-300 text-gray-900 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-red-500 transition-colors">
            ...
            <div class="flex-none w-full md:w-auto">
                <button type="submit" class="w-full md:w-auto px-6 py-2.5 bg-red-600 hover:bg-red-700 text-gray-900 font-bold rounded-lg transition-colors flex items-center justify-center border border-red-500">
    ...
    <div class="flex justify-center mt-12 gap-2">
        <?php if (($currentPage ?? $page) > 1): ?>
            <a href="?type=<?= urlencode($type) ?>&slug=<?= urlencode($slug) ?>&page=<?= ($currentPage ?? $page) - 1 ?>" class="px-4 py-2 bg-white hover:bg-red-600 text-gray-900 rounded transition-colors">Trang trước</a>
        <?php endif; ?>
        <span class="px-4 py-2 text-gray-600">Trang <?= $currentPage ?? $page ?> / <?= $totalPages ?? 1 ?></span>
        <?php if (($currentPage ?? $page) < ($totalPages ?? 1)): ?>
            <a href="?type=<?= urlencode($type) ?>&slug=<?= urlencode($slug) ?>&page=<?= ($currentPage ?? $page) + 1 ?>" class="px-4 py-2 bg-white hover:bg-red-600 text-gray-900 rounded transition-colors">Trang sau</a>
        <?php endif; ?>
    </div>
</div>
