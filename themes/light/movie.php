...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="relative w-full rounded-2xl overflow-hidden mb-12 shadow-2xl bg-gray-50">
        ...
        <div class="relative z-10 p-6 md:p-12 flex flex-col md:flex-row gap-8">
            <div class="flex-shrink-0 w-48 md:w-64 mx-auto md:mx-0">
                <img src="<?= htmlspecialchars(!empty($movie['thumb_url']) ? $movie['thumb_url'] : (!empty($movie['poster_url']) ? $movie['poster_url'] : '')) ?>" 
                     alt="Thumb" class="w-full rounded-xl shadow-2xl border-2 border-gray-300/50">
            </div>
            
            <div class="flex-grow">
                <div class="flex flex-wrap gap-2 mb-4">
                    <?php if (!empty($movie['quality'])): ?>
                        <span class="inline-flex items-center whitespace-nowrap w-fit px-3 py-1 bg-red-600 text-gray-900 text-xs font-bold rounded-md"><?= htmlspecialchars($movie['quality']) ?></span>
                    <?php endif; ?>
                    <?php if (!empty($movie['lang'])): ?>
                        <span class="inline-flex items-center whitespace-nowrap w-fit px-3 py-1 bg-blue-600 text-gray-900 text-xs font-bold rounded-md"><?= htmlspecialchars($movie['lang']) ?></span>
                    <?php endif; ?>
                    <?php if (!empty($movie['year'])): ?>
                        <span class="inline-flex items-center whitespace-nowrap w-fit px-3 py-1 bg-white text-gray-900 text-xs font-bold rounded-md"><?= htmlspecialchars($movie['year']) ?></span>
                    <?php endif; ?>
                </div>
                
                <h1 class="text-3xl md:text-5xl font-bold text-gray-900 mb-2"><?= htmlspecialchars($movie['name']) ?></h1>
                <p class="text-xl text-gray-600 mb-4 italic"><?= htmlspecialchars($movie['origin_name'] ?? '') ?></p>
                ...
                <div class="flex items-center gap-2 mb-6">
                    ...
                    <span class="text-gray-600 text-sm">(<?= number_format($tmdbCount) ?> votes)</span>
                </div>
                <?php else: ?>
                <div class="mb-6"></div>
                <?php endif; ?>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 text-sm">
                    <div class="bg-white/50 p-3 rounded-lg border border-gray-300/50">
                        <span class="text-gray-500 block mb-1">Trạng thái</span>
                        <span class="text-gray-900 font-semibold"><?= htmlspecialchars($movie['episode_current'] ?? 'N/A') ?></span>
                    </div>
                    <div class="bg-white/50 p-3 rounded-lg border border-gray-300/50">
                        <span class="text-gray-500 block mb-1">Thời lượng</span>
                        <span class="text-gray-900 font-semibold"><?= htmlspecialchars($movie['time'] ?? 'N/A') ?></span>
                    </div>
                    <div class="bg-white/50 p-3 rounded-lg border border-gray-300/50">
                        <span class="text-gray-500 block mb-1">Loại</span>
                        <span class="text-gray-900 font-semibold"><?= htmlspecialchars($movie['type'] ?? 'N/A') ?></span>
                    </div>
                </div>
                ...
                <div class="mb-8">
                    <h3 class="text-xl font-bold mb-3 text-gray-900 border-l-4 border-red-500 pl-2">Hình Ảnh Phim</h3>
                    <div class="flex overflow-x-auto gap-4 custom-scrollbar pb-4 snap-x">
                        ...
                        foreach ($movieImages['backdrops'] as $img): 
                            if ($bCount++ >= 10) break;
                        ?>
                            <div class="shrink-0 w-[240px] md:w-[280px] rounded-xl overflow-hidden border border-gray-300 snap-start">
                        ...
                        foreach ($movieImages['posters'] as $img): 
                            if ($pCount++ >= 5) break;
                        ?>
                            <div class="shrink-0 w-[100px] md:w-[120px] rounded-xl overflow-hidden border border-gray-300 snap-start">
                ...
                <?php if (!empty($episodes) && !empty($episodes[0]['server_data'])): ?>
                    <a href="/<?= $settings["slugWatch"] ?? "xem-phim" ?>/<?= urlencode($slug) ?>/<?= urlencode($episodes[0]['server_data'][0]['slug']) ?>" 
                       class="inline-flex items-center justify-center space-x-2 bg-red-600 hover:bg-red-700 text-gray-900 px-8 py-3.5 rounded-xl font-bold transition-all transform hover:scale-105 shadow-lg shadow-red-600/30">
    ...
    <?php if (!empty($episodes[0]['server_data'])): ?>
        <div class="mb-12 bg-gray-50 rounded-2xl p-6 border border-gray-200">
            ...
            <div class="flex flex-wrap gap-2 p-2">
                <?php foreach ($episodes[0]['server_data'] as $e): ?>
                    <a href="/<?= $settings["slugWatch"] ?? "xem-phim" ?>/<?= urlencode($slug) ?>/<?= urlencode($e['slug']) ?>" 
                       class="px-4 py-2 rounded-lg transition-all bg-white text-gray-700 hover:bg-red-600 hover:text-gray-900 hover:shadow-lg hover:shadow-red-600/30 font-medium">
    ...
    <div class="mb-12 bg-gray-50 rounded-2xl p-6 border border-gray-200">
        ...
        <div class="relative bg-white rounded-xl p-4 border border-gray-300">
            <input type="text" id="comment-name" class="w-full bg-transparent text-gray-900 text-sm outline-none mb-3 pb-2 border-b border-gray-300 hidden" placeholder="Nhập tên của bạn...">
            <textarea id="comment-content" rows="3" class="w-full bg-transparent text-gray-900 text-sm outline-none resize-none placeholder-gray-400" placeholder="Vui lòng nhập nội dung bình luận..."></textarea>
            <div class="flex items-center justify-between mt-3 border-t border-gray-300 pt-4">
                <label class="flex items-center text-gray-600 text-sm cursor-pointer hover:text-gray-900 transition-colors">
                    <input type="checkbox" id="comment-anon" checked class="mr-2 rounded border-gray-600 bg-gray-200 text-red-500 focus:ring-red-500"> Ẩn danh ?
                </label>
                <button id="btn-submit-comment" class="bg-red-600 text-gray-900 font-bold px-5 py-2 rounded-lg text-sm flex items-center hover:bg-red-700 transition-colors shadow-lg shadow-red-600/20">
                        ...
                        res.data.forEach(c => {
                            html += `
                                <div class="flex gap-4">
                                    <div class="w-12 h-12 rounded-full bg-white flex items-center justify-center shrink-0 border border-gray-300">
                                        <svg class="w-6 h-6 text-gray-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>
                                    </div>
                                    <div class="flex-1 bg-white/50 p-4 rounded-xl border border-gray-300/50">
    ...
    <div class="mb-12">
        <h3 class="text-2xl font-bold text-gray-900 mb-6 border-l-4 border-red-500 pl-3">Có Thể Bạn Sẽ Thích</h3>
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
            <?php foreach ($suggestions as $item): ?>
                <a href="/<?= $settings["slugMovie"] ?? "phim" ?>/<?= urlencode($item['slug']) ?>" class="group flex flex-col relative overflow-hidden rounded-xl bg-white transition-all hover:scale-105 hover:shadow-xl hover:shadow-red-500/20">
                    <div class="relative aspect-[2/3] w-full overflow-hidden">
                        ...
                        <div class="absolute top-2 left-2">
                            <span class="bg-red-600 text-gray-900 text-[10px] font-bold px-2 py-0.5 rounded shadow-sm"><?= htmlspecialchars($item['quality'] ?? 'HD') ?></span>
                        </div>
                    </div>
                    <div class="p-3 relative z-10 flex flex-col flex-grow">
                        <h3 class="text-sm font-semibold text-gray-900 line-clamp-1 mb-1 group-hover:text-red-400 transition-colors"><?= htmlspecialchars($item['name']) ?></h3>
                    </div>
                </a>
            <?php endforeach; ?>
        </div>
