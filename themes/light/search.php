...
<div class="container mx-auto px-4 py-8">
    <div class="mb-8 max-w-xl mx-auto md:hidden">
        <form action="search.php" method="GET" class="relative">
            <input type="text" name="keyword" value="<?= htmlspecialchars($keyword) ?>" placeholder="Tìm kiếm phim..." 
                class="w-full bg-white text-gray-200 text-sm rounded-full pl-10 pr-4 py-3 focus:outline-none focus:ring-2 focus:ring-red-500 border border-gray-300 transition-all">
            <i data-lucide="search" class="absolute left-4 top-3.5 w-5 h-5 text-gray-600"></i>
        </form>
    </div>

    <div class="flex items-center justify-between mb-8">
        <h2 class="text-2xl font-bold text-gray-900 border-l-4 border-red-500 pl-3"><?= $title ?></h2>
    </div>
    ...
    <?php if (empty($movies)): ?>
        <div class="text-center py-12 text-gray-600">Không tìm thấy kết quả nào phù hợp.</div>
    <?php endif; ?>
</div>
