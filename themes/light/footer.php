    ...
    <footer class="bg-gray-50 border-t border-gray-200 text-gray-600 py-12 mt-auto">
        <div class="container mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8">
            <div>
                <a href="/" class="flex items-center space-x-2 mb-4 hover:opacity-80 transition-opacity">
                    ...
                    <?php else: ?>
                        <div class="w-8 h-8 bg-blue-600 rounded flex items-center justify-center">
                            <i data-lucide="monitor-play" class="w-5 h-5 text-gray-900 fill-current"></i>
                        </div>
                    <?php endif; ?>
                ...
                <div class="flex items-center space-x-4">
                    <?php if (!empty($settings['socialFacebook'])): ?><a href="<?= htmlspecialchars($settings['socialFacebook']) ?>" target="_blank" class="text-gray-600 hover:text-blue-600 transition-colors"><i data-lucide="facebook" class="w-5 h-5"></i></a><?php endif; ?>
                    <?php if (!empty($settings['socialYoutube'])): ?><a href="<?= htmlspecialchars($settings['socialYoutube']) ?>" target="_blank" class="text-gray-600 hover:text-red-600 transition-colors"><i data-lucide="youtube" class="w-5 h-5"></i></a><?php endif; ?>
                    <?php if (!empty($settings['socialTwitter'])): ?><a href="<?= htmlspecialchars($settings['socialTwitter']) ?>" target="_blank" class="text-gray-600 hover:text-blue-500 transition-colors"><i data-lucide="twitter" class="w-5 h-5"></i></a><?php endif; ?>
                    <?php if (!empty($settings['socialTelegram'])): ?><a href="<?= htmlspecialchars($settings['socialTelegram']) ?>" target="_blank" class="text-gray-600 hover:text-blue-500 transition-colors"><i data-lucide="send" class="w-5 h-5"></i></a><?php endif; ?>
                </div>
            </div>
            
            <div>
                <h4 class="text-gray-900 font-semibold mb-4">Thể Loại</h4>
            ...
            <div>
                <h4 class="text-gray-900 font-semibold mb-4">Quốc Gia</h4>
                <ul class="space-y-2 text-sm">
            ...
            <div>
                <h4 class="text-gray-900 font-semibold mb-4">Liên Kết</h4>
                <ul class="space-y-2 text-sm">
        ...
        <div class="container mx-auto px-4 mt-8 pt-8 border-t border-gray-200 text-sm text-center">
