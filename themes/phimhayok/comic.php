...
<div class="relative w-full h-[50vh] overflow-hidden -mt-20">
    <div class="absolute inset-0">
        <img src="<?= htmlspecialchars(!empty($comic['poster_url']) ? $comic['poster_url'] : $comic['thumb_url']) ?>" alt="<?= htmlspecialchars($comic['name']) ?>" class="w-full h-full object-cover">
    ...
    <div class="flex flex-col md:flex-row gap-8">
        ...
        <div class="w-[240px] md:w-[280px] shrink-0 mx-auto md:mx-0 flex flex-col gap-6">
            ...
            <div class="rounded-xl overflow-hidden shadow-2xl border border-gray-800 relative group">
                <img src="<?= htmlspecialchars($comic['thumb_url']) ?>" alt="<?= htmlspecialchars($comic['name']) ?>" class="w-full h-auto aspect-[3/4] object-cover">
                <div class="absolute top-2 right-2">
                    <span class="bg-[#fcc526] text-black text-[11px] font-bold px-2 py-1 rounded shadow-lg">
                        <?= htmlspecialchars($comic['quality'] ?? 'HD') ?> <?= !empty($comic['lang']) ? '• '.$comic['lang'] : '' ?>
                    </span>
                </div>
            </div>
            ...
            <div class="bg-[#141414] rounded-xl p-5 border border-gray-800 text-sm text-gray-300 flex flex-col gap-3 shadow-lg">
                <div class="flex justify-between border-b border-gray-800 pb-2">
                    ...
                    <span class="text-[#fcc526] font-medium"><?= htmlspecialchars($comic['episode_current'] ?? 'Đang cập nhật') ?></span>
                </div>
                <div class="flex justify-between border-b border-gray-800 pb-2">
                    <span class="text-gray-500">Tổng tập:</span>
                    <span class="text-white"><?= htmlspecialchars($comic['episode_total'] ?? 'Đang cập nhật') ?></span>
                </div>
                <div class="flex justify-between border-b border-gray-800 pb-2">
                    <span class="text-gray-500">Thời lượng:</span>
                    <span class="text-white"><?= htmlspecialchars($comic['time'] ?? 'Đang cập nhật') ?></span>
                </div>
                <div class="flex justify-between border-b border-gray-800 pb-2">
                    <span class="text-gray-500">Ngôn ngữ:</span>
                    <span class="text-white"><?= htmlspecialchars($comic['lang'] ?? 'Vietsub') ?></span>
                </div>
                <div class="flex justify-between border-b border-gray-800 pb-2">
                    <span class="text-gray-500">Năm:</span>
                    <span class="text-white"><?= htmlspecialchars($comic['year'] ?? date('Y')) ?></span>
                </div>
                <div class="flex flex-col border-b border-gray-800 pb-2">
                    <span class="text-gray-500 mb-1">Quốc gia:</span>
                    <span class="text-white"><?= htmlspecialchars(is_array($comic['country'] ?? null) ? implode(', ', array_map(function($c) { return is_array($c) ? $c['name'] : $c; }, $comic['country'])) : ($comic['country'] ?? 'Đang cập nhật')) ?></span>
                </div>
                <div class="flex flex-col border-b border-gray-800 pb-2">
                    <span class="text-gray-500 mb-1">Đạo diễn:</span>
                    <span class="text-white"><?= htmlspecialchars(is_array($comic['director'] ?? null) ? implode(', ', $comic['director']) : ($comic['director'] ?? 'Đang cập nhật')) ?></span>
                </div>
                <div class="flex flex-col pb-1">
                    <span class="text-gray-500 mb-1">Diễn viên:</span>
                    <span class="text-white line-clamp-3 hover:line-clamp-none transition-all"><?= htmlspecialchars(is_array($comic['actor'] ?? null) ? implode(', ', $comic['actor']) : ($comic['actor'] ?? 'Đang cập nhật')) ?></span>
                </div>
            </div>
        </div>
        ...
        <div class="flex-1 text-white pt-2 md:pt-4">
            ...
            <div class="flex flex-wrap gap-2 mb-6">
                <?php 
                $cats = is_array($comic['category'] ?? null) ? $comic['category'] : [];
                foreach ($cats as $cat): 
...
<div class="mb-10" id="comments-section" data-slug="<?= htmlspecialchars($slug) ?>">
                ...
                <div class="text-gray-400 text-sm leading-relaxed whitespace-pre-line bg-[#141414] p-5 rounded-xl border border-gray-800 shadow-inner">
                    <?= nl2br(htmlspecialchars(strip_tags($comic['content'] ?? 'Chưa có nội dung mô tả cho phim này.'))) ?>
                </div>
            </div>
...
<div class="container mx-auto px-4 md:px-6 lg:px-8 max-w-[1200px] mb-16 relative z-10">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
        <?php foreach ($suggestions as $item): ?>
            <a href="/<?= $settings["slugComic"] ?? "phim" ?>/<?= urlencode($item['slug']) ?>" class="group block relative overflow-hidden rounded-xl bg-transparent transition-all duration-300">
