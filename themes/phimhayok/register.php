        ...
        <div class="max-w-md w-full bg-[#111] backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl overflow-hidden">
            ...
            <?php else: ?>
                <div class="p-8">
                    ...
                    <?php if (isset($settings['allowRegistration']) && $settings['allowRegistration'] == 0): ?>
                        <div class="mb-6 p-4 rounded-xl bg-red-500/10 border border-red-500/30 flex items-start space-x-3 text-red-400">
                            <i data-lucide="alert-circle" class="w-5 h-5 flex-shrink-0 mt-0.5"></i>
                            <p class="text-sm font-medium">Đăng ký tài khoản mới hiện đang bị khóa bởi Quản trị viên.</p>
                        </div>
                        <div class="mt-6 text-center text-gray-400 text-sm">
                            Đã có tài khoản? <a href="/login.php" class="text-phim-yellow hover:text-white font-bold transition-colors">Đăng nhập ngay</a>
                        </div>
                    <?php else: ?>
                        <?php if ($error): ?>
                            <div class="mb-6 p-4 rounded-xl bg-red-500/10 border border-red-500/30 flex items-start space-x-3 text-red-400">
