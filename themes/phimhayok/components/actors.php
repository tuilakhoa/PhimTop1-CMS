    ...
    <div class="flex overflow-x-auto gap-4 custom-scrollbar pb-4 snap-x">
        <?php foreach ($peoples as $person): ?>
            <a href="/tim-kiem?keyword=<?= urlencode($person['name'] ?? '') ?>" class="shrink-0 w-24 md:w-28 text-center snap-start group block">
                <div class="w-20 h-20 md:w-24 md:h-24 mx-auto rounded-full overflow-hidden border-2 border-gray-700 bg-gray-800 mb-2 transition-colors group-hover:border-[#fcc526]">
                    ...
                    <?php else: ?>
                        <div class="w-full h-full flex items-center justify-center">
                            <i data-lucide="user" class="w-8 h-8 text-gray-500 group-hover:text-[#fcc526] transition-colors"></i>
                        </div>
                    <?php endif; ?>
                </div>
                <h4 class="text-white text-xs font-medium line-clamp-1 group-hover:text-[#fcc526] transition-colors" title="<?= htmlspecialchars($person['name'] ?? '') ?>"><?= htmlspecialchars($person['name'] ?? '') ?></h4>
