...
<body class="<?= $bodyClass ?> min-h-screen flex flex-col">
 <!-- Header -->
 <nav class="header-solid fixed w-full top-0 z-50 duration-300 h-[72px]">
 ...
 <a href="/<?= $settings["slugList"] ?? "danh-sach" ?>/phim-bo" class=" flex items-center ">
 <i data-lucide="tv" class="w-4 h-4 mr-1.5"></i> Phim bộ
 </a>
 <a href="/<?= $settings["slugList"] ?? "danh-sach" ?>/phim-le" class=" flex items-center ">
 <i data-lucide="film" class="w-4 h-4 mr-1.5"></i> Phim lẻ
 </a>
 <button onclick="openGlobalWatchParty()" class="text-phim-yellow flex items-center font-bold">
 ...
 <button class=" flex items-center py-6">
 <i data-lucide="compass" class="w-4 h-4 mr-1.5"></i> Khám phá <i data-lucide="chevron-down" class="w-4 h-4 ml-1"></i>
 </button>
 <div class="absolute top-[100%] left-0 w-[700px] bg-[#141414] border border-gray-800 rounded-xl opacity-0 invisible group- group- z-50 p-6 flex gap-6">
 ...
 <a href="/<?= $settings["slugGenre"] ?? "the-loai" ?>/<?= htmlspecialchars($g['slug']) ?>" class="text-sm text-gray-400 px-2 py-1 rounded truncate"><?= htmlspecialchars($g['name']) ?></a>
 <?php endforeach; ?>
 </div>
 </div>
 ...
 <a href="/<?= $settings["slugCountry"] ?? "quoc-gia" ?>/<?= htmlspecialchars($c['slug']) ?>" class="text-sm text-gray-400 px-2 py-1 rounded truncate"><?= htmlspecialchars($c['name']) ?></a>
 <?php endforeach; ?>
 </div>
 </div>
 ...
 ?>
 <a href="/nam/<?= htmlspecialchars($y) ?>" class="text-sm text-gray-400 px-2 py-1 rounded text-center "><?= htmlspecialchars($y) ?></a>
 <?php endforeach; ?>
 </div>
 </div>
 ...
 <button onclick="document.getElementById('downloadAppModal').classList.remove('hidden')" class="hidden md:flex items-center bg-yellow-500 text-black text-sm font-bold py-2 px-4 rounded-full )] border border-yellow-400/50">
 ...
 <!-- Mobile Menu (Hidden) -->
 <div id="mobileMenu" class="hidden bg-[#0a0a0a] border-t border-gray-900 absolute w-full left-0 top-[72px] pb-4 max-h-[calc(100vh-72px)] overflow-y-auto">
 ...
 <a href="/<?= $settings["slugList"] ?? "danh-sach" ?>/phim-le" class=" block py-1">Phim Lẻ</a>
 <a href="/<?= $settings["slugList"] ?? "danh-sach" ?>/phim-bo" class=" block py-1">Phim Bộ</a>
 <button onclick="openGlobalWatchParty()" class="text-phim-yellow block py-1 text-left flex items-center"><i data-lucide="users" class="w-4 h-4 mr-2"></i> Xem Chung</button>
 
 <details class="group">
 <summary class="flex items-center justify-between cursor-pointer py-1 list-none [&::-webkit-details-marker]:hidden">
 <span>Thể loại</span>
 ...
 <?php foreach ($genres as $g): ?>
 <a href="/<?= $settings["slugGenre"] ?? "the-loai" ?>/<?= htmlspecialchars($g['slug']) ?>" class="text-sm text-gray-400 truncate"><?= htmlspecialchars($g['name']) ?></a>
 <?php endforeach; ?>
 </div>
 </details>
 ...
 <div class="grid grid-cols-2 gap-2 mt-2 mb-2 pl-4 border-l border-gray-800">
 <?php foreach ($countries as $c): ?>
 <a href="/<?= $settings["slugCountry"] ?? "quoc-gia" ?>/<?= htmlspecialchars($c['slug']) ?>" class="text-sm text-gray-400 truncate"><?= htmlspecialchars($c['name']) ?></a>
 <?php endforeach; ?>
 </div>
 </details>
 ...
 <a href="/nam/<?= $y ?>" class="text-sm text-gray-400 "><?= $y ?></a>
 <?php endfor; ?>
 </div>
 </details>
 ...
 <?php if (!empty($settings['appDownloadUrl']) || !empty($settings['appDownloadUrlTv']) || !empty($settings['appDownloadUrlWindows']) || !empty($settings['appDownloadUrlLinux']) || !empty($settings['appDownloadUrlFedora']) || !empty($settings['appDownloadUrlUbuntu'])): ?>
 <button onclick="document.getElementById('downloadAppModal').classList.remove('hidden')" class="flex w-full items-center justify-center bg-yellow-500 text-black font-bold py-2.5 px-4 rounded-lg mt-2">
 ...
 <!-- Download App Modal -->
 <div id="downloadAppModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop- p-4 ">
 <div class="bg-black/80 backdrop- border border-white/10 rounded-2xl w-full max-w-sm overflow-hidden relative">
 <button onclick="document.getElementById('downloadAppModal').classList.add('hidden')" class="absolute top-3 right-3 text-gray-400 bg-white/5 rounded-full p-1.5 z-10">
 ...
 <a href="<?= htmlspecialchars($settings['appDownloadUrl']) ?>" target="_blank" class="flex items-center justify-between bg-phim-yellow text-black font-semibold py-2.5 px-4 rounded-xl ">
 ...
 <a href="<?= htmlspecialchars($settings['appDownloadUrlTv']) ?>" target="_blank" class="flex items-center justify-between bg-white/5 text-gray-200 font-medium py-2.5 px-4 rounded-xl border border-white/10">
 <div class="flex items-center text-sm">
 ...
 <a href="<?= htmlspecialchars($settings['appDownloadUrlWindows']) ?>" target="_blank" class="flex items-center justify-between bg-blue-600/20 text-blue-400 font-medium py-2.5 px-4 rounded-xl border border-blue-500/30">
 <div class="flex items-center text-sm">
 ...
 container.className = 'absolute top-full left-0 mt-2 w-full bg-[#141414] border border-gray-800 rounded-xl z-[100] overflow-hidden hidden';
 ...
 // Show loading
 container.innerHTML = `<div class="p-4 text-center text-gray-500 text-sm flex items-center justify-center gap-2"><i data-lucide="loader-2" class="w-4 h-4 > Đang tìm...</div>`;
 ...
 html += `
 <a href="/phim/${item.slug}" class="flex items-center px-4 py-2 gap-3 group">
 ...
 <div class="text-gray-200 text-sm font-medium truncate group- ">${item.name}</div>
 ...
 html += `
 <a href="/search?keyword=${encodeURIComponent(q)}" class="block px-4 py-3 text-center text-sm text-phim-yellow font-medium border-t border-gray-800 mt-2">
