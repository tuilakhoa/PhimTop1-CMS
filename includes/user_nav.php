...
<div class="flex items-center space-x-4">
 ...
 <button class="relative p-2 text-gray-300 focus:outline-none" id="btn-notifications">
 ...
 <div class="absolute right-0 mt-2 w-80 bg-gray-900 border border-gray-800 rounded-xl opacity-0 invisible group- group- origin-top-right transform scale-95 group- z-50">
 <div class="p-3 border-b border-gray-800 flex justify-between items-center">
 <h4 class="text-white font-bold">Thông Báo</h4>
 <button id="btn-mark-all-read" class="text-xs text-blue-400 tất cả</button>
 </div>
 ...
 <div class="relative group z-50">
 <button class="flex items-center focus:outline-none bg-gray-800/50 px-2 py-1.5 md:px-3 md:py-2 rounded-full border border-gray-700/50">
 <div class="relative w-6 h-6 md:w-8 md:h-8 mr-1 md:mr-2">
 <?php
 $userName = !empty($_SESSION['user']['name']) ? $_SESSION['user']['name'] : 'User';
 $avatarUrl = !empty($_SESSION['user']['avatar']) ? $_SESSION['user']['avatar'] : 'https://ui-avatars.com/api/?name=' . urlencode($userName) . '&background=random';
 ?>
 <img id="nav-user-avatar" src="<?= htmlspecialchars($avatarUrl) ?>" class="w-full h-full rounded-full " alt="Avatar">
 ...
 <div class="absolute right-0 mt-2 w-56 bg-gray-900 border border-gray-800 rounded-xl opacity-0 invisible group- group- origin-top-right transform scale-95 group- z-50">
 <div class="p-2">
 ...
 <a href="/profiles.php" class="flex items-center px-4 py-2 text-sm text-gray-300 rounded-lg mb-1">
 <i data-lucide="users" class="w-4 h-4 mr-2"></i> Hồ sơ người xem
 </a>
 <a href="/history.php" class="flex items-center px-4 py-2 text-sm text-gray-300 rounded-lg mb-1">
 <i data-lucide="history" class="w-4 h-4 mr-2"></i> Lịch sử xem phim
 </a>
 <a href="/follows.php" class="flex items-center px-4 py-2 text-sm text-gray-300 rounded-lg mb-1">
 <i data-lucide="bookmark" class="w-4 h-4 mr-2 text-phim-yellow"></i> Phim đang theo dõi
 </a>
 <a href="/shop.php" class="flex items-center px-4 py-2 text-sm text-gray-300 rounded-lg mb-1">
 <i data-lucide="store" class="w-4 h-4 mr-2 text-red-400"></i> Cửa hàng vật phẩm
 </a>
 <button onclick="if(typeof openGlobalWatchParty === 'function') openGlobalWatchParty(); else alert('Tính năng chưa sẵn sàng!');" class="w-full text-left flex items-center px-4 py-2 text-sm text-gray-300 rounded-lg mb-1">
 <i data-lucide="users" class="w-4 h-4 mr-2 text-[#8B5CF6]"></i> Vào phòng xem chung
 </button>
 <a href="/api/auth.php?action=logout" class="flex items-center px-4 py-2 text-sm text-red-400 rounded-lg ">
 ...
 ?>
 <?php if ($canLogin): ?>
 <a href="/login.php" class="text-phim-yellow ">Đăng nhập</a>
 <?php endif; ?>
 ...
 <?php if ($canRegister): ?>
 <a href="/register.php" class="text-gray-300 ">Đăng ký</a>
 <?php endif; ?>
 </div>
 <?php endif; ?>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
 ...
 html += `
 <div class="p-3 mb-1 rounded-lg cursor-pointer ${bgClass}" onclick="markRead(${item.id})">
