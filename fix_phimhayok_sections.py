...
# Replace Phimhayok with siteName
content = content.replace('Mới Nhất Trên Phimhayok', 'Mới Nhất Trên <?= htmlspecialchars($siteName) ?>')
...
?>
"""
content = content.replace('?>\n\n<?php if (!empty($featuredMovies)): ?>', data_vars + '\n<?php if (!empty($featuredMovies)): ?>')
content = content.replace('?>\n<?php if (!empty($featuredMovies)): ?>', data_vars + '\n<?php if (!empty($featuredMovies)): ?>') # fallback
...
new_sections = make_section("Phim", "ÂU MỸ", "text-cyan-400", "quoc-gia", "au-my", "$hanQuocData")
new_sections = make_section("Phim", "TRUNG QUỐC", "text-red-500", "quoc-gia", "trung-quoc", "$trungQuocData") + "\n" + make_section("Phim", "HÀN QUỐC", "text-cyan-400", "quoc-gia", "han-quoc", "$hanQuocData")

content = content.replace('<!-- 5. Bảng Xếp Hạng (Leaderboard) -->', new_sections + '\n    <!-- 5. Bảng Xếp Hạng (Leaderboard) -->')
