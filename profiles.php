    ...
    <script>
        tailwind.config = {
            ...
            theme: {
                extend: {
                    colors: {
                        primary: '#<?= $settings['appColor'] ?? 'e50914' ?>',
                    }
                }
            }
        }
...
<body class="bg-[#141414] text-white min-h-screen flex flex-col items-center justify-center font-sans relative">
    ...
    <div id="create-modal" class="fixed inset-0 bg-black/90 hidden items-center justify-center z-50">
        <div class="bg-[#141414] p-8 max-w-xl w-full">
            ...
            if (profiles.length < 5 && !isManageMode) {
                html += `
                    <div class="profile-card cursor-pointer w-24 md:w-36 flex flex-col items-center" onclick="showCreateModal()">
                        <div class="w-24 h-24 md:w-36 md:h-36 rounded-md border-2 border-transparent hover:border-white transition-colors hover:bg-gray-200 flex items-center justify-center group bg-[#141414]">
